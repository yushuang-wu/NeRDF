...
def train():
    ...
    if args.dataset_type == 'llff':
        images, poses, bds, _, i_test, _ = load_llff_data(args.datadir, args.factor,
        ...
        print('Loaded llff', images.shape, img4test.shape, render_poses.shape, hwf, args.datadir)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, disps = render_path(render_poses, hwf4test, args.chunk, render_kwargs_test, gt_imgs=images, savedir=testsavedir)
            print('Done rendering', testsavedir)
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=30, quality=8)
            imageio.mimwrite(os.path.join(testsavedir, 'disp.mp4'), to8b(disps / np.max(disps)), fps=30, quality=8)

            if args.render_test:
                # test_loss = img2mse(torch.Tensor(rgbs), torch.Tensor(images).cuda())
                # test_psnr = mse2psnr(test_loss)
                # tqdm.write(f"Test PSNR: {test_psnr.item()}")
                # torch.save(test_psnr, f'{basedir}/{expname}/{str(test_psnr.item())[:5]}.psnr')
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(render_poses, hwf4test, args.chunk, render_kwargs_test)
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            os.makedirs(testsavedir, exist_ok=True)
            print('Test images shape for video', img4test.shape, 'Poses shape', pose4test[i_test].shape)
            
            with torch.no_grad():
                rgbs, disps = render_path(torch.Tensor(pose4test[i_test]).to(device), hwf4test, args.chunk, render_kwargs_test, gt_imgs=img4test[i_test], savedir=testsavedir)
            ...
            # torch.save(test_psnr, f'{basedir}/{expname}/{i}_{str(psnr.item())[:5]}_{str(test_psnr.item())[:5]}.psnr')
