...
def render_path(epoch, render_poses, hwf, chunk, N_gauss, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if savedir is not None:
            ...
            rgb8 = to8b(rgbs_t[-1])
            filename = os.path.join(savedir, '{:03d}_t.png'.format(i))
            imageio.imwrite(filename, rgb8)
...
def create_nerf(args):
    ...
    gauss = model_gs(D=args.netdepth_gs, W=args.netwidth_gs,
                     input_ch=input_ch, output_ch=args.N_gauss*3*4, skips=skips).to(device)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        loss = img_loss1# + img_loss2 + img_loss3 + con_loss
