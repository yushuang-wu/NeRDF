...
# @mst
def get_rand_pose(args4randpose):
    # -- pass local variables
    c2w, up, focal, poses = args4randpose
    ...
    c = np.dot(
        c2w[:3, :4],
        np.array([
            rand_uniform(mins_o[0], maxs_o[0], scale=1.0),
            rand_uniform(mins_o[1], maxs_o[1], scale=1.0),
            rand_uniform(mins_o[2], maxs_o[2], scale=1.0), 1
        ]))
    z = np.dot(
        c2w[:3, :4],
        np.array([
            rand_uniform(mins_d[0], maxs_d[0], scale=1.0),
            rand_uniform(mins_d[1], maxs_d[1], scale=1.0),
            rand_uniform(mins_d[2], maxs_d[2], scale=1.0), 1
        ]))
    z = normalize(z)
...
def load_llff_data(basedir,
    ...
    if spherify:
        ...
        args4randpose = c2w, up, focal, poses
    ...
    #     render_poses), i_test
    return images, poses, bds, render_poses, i_test, args4randpose
